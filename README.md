...
**Anki integration** — the tutor checks AnkiConnect before creating cards (no duplicates), creates cards directly into your deck, and syncs with AnkiWeb at the start of each session.
...
### For the Anki snapshot (vocabulary-aware prompts)
- The [cn-learning](https://github.com/rd11490/cn-learning) companion repo, which has a `skills/get-learned-words` script that generates the `anki-snapshot.md` file. Without it, the tutor still works but won't know your existing vocabulary.

### For the dictionary (word hover)
- The **CC-CEDICT** dictionary file (`cedict_ts.u8`). Available from [cc-cedict.org](https://cc-cedict.org/wiki/start). The cn-learning repo also ships it at `web/data/cedict_ts.u8`. Set `CEDICT_PATH` in `.env` to point to it.
...
ANKI_SNAPSHOT_PATH=/path/to/anki-snapshot.md
WEB_PORT=3001
```

`CEDICT_PATH` and `ANKI_SNAPSHOT_PATH` can point to files in the cn-learning repo if you have it cloned, or anywhere on your machine. Both are optional — the app starts without them, just with reduced functionality.
...
│   ├── tools/
│   │   ├── anki.ts            # AnkiConnect wrapper (sync, check, create card)
...
The CC-CEDICT dictionary (121k+ entries) is loaded into memory at server startup. On first page load, the browser fetches `GET /api/dictionary/all` — a pre-gzipped JSON blob (~3.5MB compressed) served with a 24-hour cache header. After that first load:
